def aspect_ratio(x_corners,y_corners):
    import time
    ...
    # load model
    case_name = "large"
    MAIN_DIR = "/data/proy/training_8November/ML_micro-main/3_predict/"
    ML_filename = "/data/proy/model_state_dict_9"
    ...
    #print(x_corners)
    y_corners = y_corners
    z_corners = [0.1E-03, 0.1E-03]
    # x_pos: [0.18E-03, 0.28E-03]
    # y_pos: [0.18E-03, 0.18E-03]
    # z_pos: [0.1E-03, 0.1E-03]
    power_control = [1,1][:-1]
    ...
    for i in range(len(x_corners) - 1):
        ...
        traveled_time = traveled_dist/vel
        #print("The unit direction",unit_direction )
        # print("The traveled time",traveled_time )
        # print("t_pre", t_pre)
        ts_seg = np.arange(t_pre, t_pre + traveled_time, dt)
    ...
    def get_T_laser(centroids, x_laser, y_laser, z_laser, power, unit_mov_dir):
        ...
        T = T_ambient + Q / (2 * np.pi * kappa) / R * np.exp(-vel / (2*alpha) * (R + projection))
    ...
    Ny = 464*2
    Nz = 46
    cell_size = 2e-3/Nx  #422.0  ### Always change this depending on the size on NX
    div = 928000#1/cell_size #422000 # pixels to meters needs modification all the time depending on the size on NX
    ...
    large_array = np.load(os.path.join(MAIN_DIR, 'stacked_array.npy'))
    #print(large_array.shape)
    ...
    ### (len(VOI)/2)*40
    T0 = get_T_laser(centroids, x_laser, y_laser, z_laser, ps[0], mov_dir[0]) 
    T1 = get_T_laser(centroids, x_laser, y_laser, z_laser, ps[0+skip], mov_dir[0+skip])  
    ...
    try:
        for i in range(0, max_t-skip, skip):
            ...
            T0 = get_T_laser(centroids, x_laser+shift[0]/div, y_laser+shift[1]/div, z_laser, ps[i], mov_dir[i]) 
            T1 = get_T_laser(centroids, x_laser+laser_mov[0]+shift[0]/div, y_laser+laser_mov[1]+shift[1]/div, z_laser, ps[i+skip], mov_dir[i+skip]) 
            T0 = T0.reshape((80, 80, 32), order="F")
            T1 = T1.reshape((80, 80, 32), order="F")
        ...
        # print(t_I0)
            ...
            if i % 100 == 0:
                np.save(os.path.join("/data/proy/training_8November/ML_micro-main/3_predict/data/example/neper", f'process_eta'), l_array.astype(np.int8))
            #print(tuple_list)
        ...
        f = '/data/proy/training_8November/ML_micro-main/3_predict/data/example/neper/process_eta'
        id = np.load(f+'.npy')
...
# Define the grid dimensions and corner coordinates
grid_size = 11
x_min, x_max = 0.2, 1.8
y_min, y_max = 0.2, 1.8
...
# Initialize a list to store the movements
movement_list = []

# Iterate over each grid point and calculate the possible movements
for point in grid_points:
    x0, y0 = point
    for movement in movements:
        dx, dy = movement
        x1, y1 = x0 + dx * (x_max - x_min) / (grid_size - 1), y0 + dy * (y_max - y_min) / (grid_size - 1)
        
        # Ignore movements based on the given conditions
        #if x0 >= 0.20 and x0<=0.80 and x1>= .20 and x1<=0.80 and y0 >= 0.20 and y0 <=0.80 and y1>= 0.20 and y1 <=0.80:
        #if x1>= .20 and x1<=0.80 and y1>= .20:
        if x0>= .20 and x0<=1.80:# and x1<=0.80 and x1>= .20:    
            # x0=x0/1000
            # x1=x1/1000
            # y0=y0/1000
            # y1=y1/1000
            ...
            x_corners=[x0/1000,x1/1000]
            y_corners=[y0/1000,y1/1000]
            print(x_corners,y_corners)
            AR= aspect_ratio(x_corners,y_corners)
            # Append the movement to the list
            movement_list.append([[x0, y0], [x1, y1],AR])
                
print(len(movement_list))
# Save the list of movements to a JSON file
with open('movements.json', 'w') as f:
    json.dump(movement_list, f)
# Save the list of movements to a text file
