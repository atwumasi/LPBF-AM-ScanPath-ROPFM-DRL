...
MAIN_DIR = "/media/acsmil/Disk 1/acsmil/training_8November/ML_micro-main/1_sample"
...
for ID in IDS:
    for Q in QS:
        ...
        ind_path = f"/media/acsmil/Disk 1/acsmil/training_8November/ML_micro-main/1_sample/validation data/{case_name}"
        ...
        def rve_picker(x, y, z, large_array):
            ...
            #rve = large_array[x-63:x+27, y-27:y+28, z-28:z]
            rve = large_array[x-60:x+20, y-27:y+53, z-32:z]
            return rve
        ...
        for file in os.listdir(ind_path):
            if file.startswith("cell_ori_ind"):
                inds.append(file)
        ...
        for i, ind in enumerate(inds[:-1]):
            ...
            # mask = I1
            np.save(os.path.join(MAIN_DIR, "val_images", f"{case_name}_image_{i+1}"), image.astype(np.float32))
            np.save(os.path.join(MAIN_DIR, "val_masks", f"{case_name}_mask_{i+1}"), I1.astype(np.int8))
